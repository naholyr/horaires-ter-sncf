...
class client_termobile implements client_interface
{
  ...
  {
    return curl_getinfo($this->s, CURLINFO_EFFECTIVE_URL);
  }

  // Get URL adding parameters and retrieve html/status
  private function do_get($url, $params, &$status)
  {
    curl_setopt($this->s, CURLOPT_COOKIE, $this->cookie);
    curl_setopt($this->s, CURLOPT_URL, $url.'?'.http_build_query($params));
  ...
  public function cherche_gare(&$nom_gare, $id_gare)
  {
    ...
    if (count($gares) == 0) {
      ...
      $nom_gare_sans_accent = nom_gare(strtolower(iconv("UTF-8", "ASCII//TRANSLIT", $nom_gare)));
      $this->fixNomGare($nom_gare_sans_accent);
      foreach ($gares as $nom => $id) {
        $nom_sans_accent = nom_gare(strtolower(iconv("UTF-8", "ASCII//TRANSLIT", $nom)));
        $this->fixNomGare($nom_sans_accent);
  ...
  public function cherche_train($num)
  {
    ...
    $info = array();
    $html = str_replace(array("\r", "\n", '&nbsp;', "\t"), array('', '', ' ', ' '), $html);
    if (preg_match('#<table.*? id=[\'"]content[\'"].*?> *(.+) *</table>#i', $html, $match)) {
  ...
  {
    $type_sans_accent = iconv("UTF-8", "ASCII//TRANSLIT", $type);
    if (preg_match('/lunea/i', $type)) {
      $type = 'Lunéa';
    } elseif (preg_match('/corail/i', $type)) {
      $type = 'Corail';
    } elseif (preg_match('/train ter/i', $type)) {
      $type = 'TER';
    } elseif (preg_match('/car ter/i', $type)) {
      $type = 'Car';
    }
  }
